const database=[
  ...
  const deathMoreFiveDatabase = database.filter(function(element,index) {  // filter มีหน้าที่เป็นหน่วยคัดกรอง
      return element.total_death > 5 && element.province !== 'ทั้งประเทศ'
  }).map(function(element,index){                                          // map สร้าง Arrayและvalueตามเงื่อนไขที่เราต้องการ
      return({
  ...
  //    return({
  //        province:element.province,
  //        total_death:element.total_death                                 // หาจำนวนในคนเสียชีวิตเต่ละจังหวัด
  //    })
  // })
...
const newsData = newsdatabase.results.filter(function(element, index){
